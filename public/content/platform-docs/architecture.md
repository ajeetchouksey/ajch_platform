...
| Platform stats | `.json` | `stats.json` | Live counters (auto-generated by build-content-intelligence.mjs) |
...
## Stats & Relationships Pipeline

`scripts/build-content-intelligence.mjs` regenerates both `public/content/stats.json` and `public/content/relationships.json` — trusting each vertical's own declared counts (a promoted vertical's `catalog.json`/`index.json`) rather than re-deriving them independently:

- `blog/index.json` → non-draft post count
- `skillup/catalog.json` → question/exam/note/scenario counts (aggregated from each exam's own declared fields)
- `usecases/_source-intel.json` → deduped use-case count
- `hol-labs/index.json` → `totalCount`
- `.claude/agents/*.md` → agent count
- `src/app/router.tsx` → tool sub-route count (regex match on `/tools/*`)
- `relationships.json`: every cross-type doc pair scored by shared `taxonomyIds` (see `public/content/taxonomy.json`), weighted by recency

The home page (`HomeV2.tsx`) loads `stats.json` on mount and uses the values to populate the proof-bar stat cards dynamically. Regeneration is automatic — a `repository_dispatch` from each vertical repo's own CI (near-real-time) plus a 6-hourly scheduled safety net (`content-intelligence-sync.yml`), and inline as part of every promotion (`promote-content.yml`).
...
4. **Content Sync** — `build-content-intelligence.mjs` runs after any content write to update `stats.json` and `relationships.json`
