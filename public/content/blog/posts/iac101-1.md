...
excerpt: "IaC - Understand ARM Template"
...
```mermaid
flowchart TD
  VS["Visual Studio 2017"] --> FILE["File → New Project"]
  FILE --> CLOUD["Installed → Templates → Visual C# → Cloud"]
  CLOUD --> ARG["Azure Resource Group\nVisual C#\nDeployment project for Azure ARM"]
  ARG --> CONFIG["Name: AzureResourceGroup1\nLocation: C:/Code/\nCreate directory for solution: true"]
  CONFIG --> OK["OK → Create project"]
```
...
```mermaid
flowchart TD
  DLG["Select Azure Template"] --> SRC["Show templates from: Visual Studio Templates"]
  SRC --> T1["Blank Template - MICROSOFT"]
  SRC --> T2["Windows Virtual Machine Scale Set"]
  SRC --> T3["Linux Virtual Machine Scale Set"]
  SRC --> T4["Service Fabric Cluster"]
  SRC --> T5["Web app"]
  SRC --> T6["Web app + SQL"]
  SRC --> T7["Logic App"]
  T1 --> DESC["Blank Template\nAdds azuredeploy.json + deployment files\nfor Azure Resource Manager"]
```
...
```mermaid
graph TD
  SOL["Solution 'AzureResourceGroup1'\n1 project"] --> PRJ["AzureResourceGroup1"]
  PRJ --> REF["References"]
  PRJ --> DEPLOY["azuredeploy.json\nARM template orchestration"]
  PRJ --> PARAMS["azuredeploy.parameters.json\nParameter values for deployment"]
  PRJ --> PS["Deploy-AzureResourceGroup.ps1\nValidation + artifact staging script"]
```
