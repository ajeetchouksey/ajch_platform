...
import {
  GitPullRequest, Lock, RefreshCw, ChevronLeft, ChevronRight,
...
export default function IssueBoard() {
  ...
  // Issues -- all setState in .then() callbacks, loading state set in click handlers
  useEffect(() => {
    const params = new URLSearchParams({ per_page: String(PER_PAGE), page: String(page) });
    params.set('state', filters.state === 'all' ? 'all' : filters.state);
    if (filters.label) params.set('labels', filters.label);
    if (filters.milestone) params.set('milestone', filters.milestone);

    fetch(`https://api.github.com/repos/${REPO}/issues?${params.toString()}`,
      { headers: apiHeaders(adminToken) })
      .then(r => r.ok ? r.json() as Promise<GhIssue[]> : [])
      .then((data: GhIssue[]) => {
        const issuesOnly = data.filter(i => !i.pull_request);
        setIssues(issuesOnly);
        setHasMore(issuesOnly.length === PER_PAGE);
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, [filters, page, adminToken]);
  ...
  // Filter handler -- initializes loading state before the issues effect fires
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-4 items-center" ref={dropdownRef}>
        ...
        <button onClick={() => { setLoading(true); setPage(1); pFilter({}); }} disabled={loading}
            ...
            <div className="flex gap-2 pt-2 justify-center">
              {page > 1 && (
                <button onClick={() => goToPage(page - 1)}
                  className="flex items-center gap-1 px-3 py-1.5 text-xs text-slate-400 hover:text-slate-200 border border-slate-700 rounded-lg transition-colors">
