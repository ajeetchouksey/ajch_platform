...
> **UseCase content moved.** Use-case content lives in `ajeetchouksey/ajch_ai_usecases`. When invoked from a session rooted in `ajch_platform`, resolve `{repo root}` below from `.claude/vertical-registry.json` → `usecases.localCheckoutWindows` — read existing cases and the index from there, not from this repo (`public/content/usecases/` no longer exists here). When invoked from a session already rooted in `ajch_ai_usecases`, `{repo root}` is the current repo root.
...
1. Read 2-3 existing case files in `{repo root}/content/usecases/cases/` closest to the brief's vertical or pattern, to match voice and depth (e.g. `loan-application-underwriting.json` for banking/HITL shape).
2. Read `{repo root}/content/usecases/index.json` — confirm the target `vertical` id exists, pick real `patterns[]` ids from the existing list (don't invent new ones without flagging it), and check `featuredIds` / existing case ids to avoid duplicating a scenario that's already covered.
3. Grep across `{repo root}/content/usecases/cases/*.json` for the brief's core scenario keywords to confirm this is genuinely new ground, not a near-duplicate of an existing case.
...
```
{
  ...
  "relatedInterviewQs": ["existing q-ids from ajch_platform's interview question bank, only if that repo is reachable in this session and a genuine match exists — omit the field entirely if ajch_platform isn't reachable, or if no genuine match exists"],
...
- Don't claim a `relatedInterviewQs` match without having actually found the question id in `ajch_platform`'s interview question bank in this session — if that repo isn't reachable, omit the field, don't guess.
