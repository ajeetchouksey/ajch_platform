...
5. Update `content/hol-labs/index.json`:
   ...
   - Append each lab's `HolLabSummary` entry into `labs[]` — `{ id, title, tagline, domain, difficulty, estimatedMinutes, costTier, tags, taxonomyIds, relatedExamIds, relatedUseCaseIds, relatedBlogSlugs, relatedLabIds }`, where `tagline` is copied verbatim from the full lab JSON (it's the catalog-card hook — never leave it out or duplicate the title), `taxonomyIds` is the same copy of `tags` you wrote onto the full lab file in step 4, and the four `relatedXIds` arrays are flattened directly from the full lab JSON's relation arrays (`relatedExams[].exam`, `relatedUseCases[].id`, `relatedBlogPosts[].slug`, `relatedLabs[].id`) — this flattened summary is what the platform's `useRelatedLabs` hook (and the cross-vertical relationship engine) reads for reverse-link lookups, so it must never be skipped or left stale
