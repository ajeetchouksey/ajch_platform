...
5. Update `content/hol-labs/index.json`:
   ...
   - Append each lab's `HolLabSummary` entry into `labs[]` — `{ id, title, domain, difficulty, estimatedMinutes, costTier, tags, relatedExamIds, relatedUseCaseIds, relatedBlogSlugs, relatedLabIds }`, where the four `relatedXIds` arrays are flattened directly from the full lab JSON's relation arrays (`relatedExams[].exam`, `relatedUseCases[].id`, `relatedBlogPosts[].slug`, `relatedLabs[].id`) — this flattened summary is what the platform's `useRelatedLabs` hook reads for reverse-link lookups, so it must never be skipped or left stale
