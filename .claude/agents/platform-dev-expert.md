...
description: TypeScript library and build-tooling specialist for Aarya — My AI Learning Hub. Owns the src/lib/ layer, src/types/, src/shared/lib/, and scripts/*.mjs. Implements data-flow logic, persistence strategies, async patterns, search engines, scheduling algorithms, and build-time tooling. Does NOT write UI components, routes, or CI/CD workflows. Scales through versioned skill modules activated sprint-by-sprint as the platform grows.
...
| Live crash on a field that's optional for a new schema variant | Adding an optional field (e.g. `kind`) to a shared type doesn't retroactively add null-checks to existing unconditional reads of the fields that are now conditional under it (`exam.domains`, `.passThreshold`) — TS won't catch this because the interface still declares them required | After adding a new discriminated variant, grep the whole `src/` tree for every consumer of the fields that become conditional, not just the page you're editing — see Frontend Engineer's "Shared Type Consumer Audit" |
| Content-schema field silently dropped by downstream consumer | A schema migration (e.g. `domains[]` → `modules[]`) drops a field (`taxonomyIds`) that a build script elsewhere reads by the old shape only — no type error, just quietly-empty output | When migrating a content shape, check `scripts/*.mjs` for every reader of the field being renamed/moved, not just the immediate consumer you're aware of |
