...
- Report final result: files written, `index.json` counts before/after, any new domain entries, cost-tier mix of the batch (flag if a batch skews too many "paid" labs), and that the change lives in the `ajch_hol_labs` working tree and still needs a commit/push/PR. **Then separately flag**: even after that PR merges, the lab isn't live until promoted — see `vertical-pipeline` skill's "Going live" section (`gh workflow run promote-content.yml`). A merged content PR is not the same as done — this exact gap shipped a lab invisibly once already.
